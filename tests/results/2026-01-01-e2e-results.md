...
### 4. `createdIndices` empty in batch_create_tracks (FIXED 2026-08-19)
**Symptom:** When creating tracks, the response includes `createdIndices: []` instead of actual indices.
**Status at the time this results file was written:** Deferred as low priority - Bitwig's `createInstrumentTrack()` doesn't return the created index. Would require polling or observer pattern to detect new track.
**Update:** Fixed in `mcp-server/src/handlers/tracks.ts` by snapshotting `track.list` before creation and diffing against a post-create re-query when Bitwig doesn't hand back an index - see CHANGELOG.md's 2026-08-19 entries for the fix and its live verification. This section is left as-written below for historical accuracy; only this note reflects current status.
